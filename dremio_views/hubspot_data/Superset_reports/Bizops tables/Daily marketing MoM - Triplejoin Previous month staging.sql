WITH FlattenedTable2 AS (
    ...
    FROM 
        "hubspot_data"."Superset_reports"."Bizops tables"."Occurred Meetings Staging"
)
SELECT 
...
LEFT JOIN 
    FlattenedTable2 AS b
ON 
    MQL.hs_record_id = b.contact_id
WHERE 
    LKN.start_ts_day >= DATE_SUB(CURRENT_DATE, 120) 
    AND LKN.start_ts_day < DATE_SUB(CURRENT_DATE, 60)
GROUP BY 
    LKN.campaignname
ORDER BY 
